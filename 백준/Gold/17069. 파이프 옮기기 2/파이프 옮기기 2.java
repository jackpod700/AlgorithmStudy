...
public class Main{
	...
	public static void main(String[] args) throws IOException{
		...
		d=new long[N][N][3];//0:가로,1:세로,2:대각선
		for(int i=0;i<N;i++) {
			for(int j=0;j<N;j++) {
				d[i][j][0]=d[i][j][1]=d[i][j][2]=Integer.MAX_VALUE;
			}
		}
		for(int i=0;i<N;i++) {
		...
		d[0][0][0]=d[0][0][1]=d[0][0][2]=0;
		d[0][1][0]=1;
		System.out.println(dp(N-1,N-1,0)+dp(N-1,N-1,1)+dp(N-1,N-1,2));
	}
	
	static long dp(int r,int c,int stat) {
		if(!check(r,c)||d[r][c][stat]==-1)return 0;
		if(d[r][c][stat]!=Integer.MAX_VALUE||(r==0&&c==0))return d[r][c][stat];
		if(stat==0) {
